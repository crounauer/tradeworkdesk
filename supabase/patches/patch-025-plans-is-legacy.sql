...
-- Step 2: Mark old tiered plans as legacy
...
-- Step 3: Create the new Base Plan (idempotent — skip if a non-legacy active plan named 'Base Plan' already exists)
...
-- Step 6: Subscribe all tenants to all active add-ons (skip duplicates)
INSERT INTO tenant_addons (tenant_id, addon_id, is_active, quantity, activated_at)
SELECT t.id, a.id, true, 1, NOW()
FROM tenants t
CROSS JOIN addons a
WHERE a.is_active = true
ON CONFLICT (tenant_id, addon_id) DO UPDATE SET is_active = true;
