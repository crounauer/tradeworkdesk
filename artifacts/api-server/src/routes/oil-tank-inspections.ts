...
router.post("/oil-tank-inspections", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const { data, error } = await supabaseAdmin.from("oil_tank_inspections").insert({ ...parsed.data, tenant_id: req.tenantId }).select().single();
...
router.get("/oil-tank-inspections/:id", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  let getQ = supabaseAdmin.from("oil_tank_inspections").select("*").eq("id", params.data.id);
...
router.patch("/oil-tank-inspections/:id", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  let existQ = supabaseAdmin.from("oil_tank_inspections").select("job_id").eq("id", params.data.id);
...
router.get("/oil-tank-inspections/job/:jobId", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  let jobQ = supabaseAdmin.from("oil_tank_inspections").select("*").eq("job_id", params.data.jobId);
...
router.delete("/oil-tank-inspections/:id", requireAuth, requireTenant, requireRole("admin"), async (req: AuthenticatedRequest, res): Promise<void> => {
  const id = req.params.id;
  let q = supabaseAdmin.from("oil_tank_inspections").select("job_id").eq("id", id);
  if (req.tenantId) q = q.eq("tenant_id", req.tenantId);
  const { data: existing } = await q.single();
  if (!existing) { res.status(404).json({ error: "Record not found" }); return; }
  const { error } = await supabaseAdmin.from("oil_tank_inspections").delete().eq("id", id);
  if (error) { res.status(500).json({ error: error.message }); return; }
