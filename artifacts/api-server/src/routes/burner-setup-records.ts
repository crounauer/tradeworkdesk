...
import {
  ...
  GetBurnerSetupRecordResponse,
  UpdateBurnerSetupRecordParams,
  UpdateBurnerSetupRecordBody,
  UpdateBurnerSetupRecordResponse,
  GetBurnerSetupRecordByJobParams,
  GetBurnerSetupRecordByJobResponse,
...
router.post("/burner-setup-records", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
...
router.get("/burner-setup-records/:id", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  const params = GetBurnerSetupRecordParams.safeParse(req.params);
  if (!params.success) { res.status(400).json({ error: params.error.message }); return; }

  let getQ = supabaseAdmin.from("burner_setup_records").select("*").eq("id", params.data.id);
  if (req.tenantId) getQ = getQ.eq("tenant_id", req.tenantId);
  const { data, error } = await getQ.single();
  if (error || !data) { res.status(404).json({ error: "Burner setup record not found" }); return; }
...
router.patch("/burner-setup-records/:id", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  let existQ = supabaseAdmin.from("burner_setup_records").select("job_id").eq("id", params.data.id);
  if (req.tenantId) existQ = existQ.eq("tenant_id", req.tenantId);
  const { data: existing } = await existQ.single();
  if (!existing) { res.status(404).json({ error: "Burner setup record not found" }); return; }
...
router.get("/burner-setup-records/job/:jobId", requireAuth, requireTenant, requirePlanFeature("oil_tank_forms"), async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  let jobQ = supabaseAdmin.from("burner_setup_records").select("*").eq("job_id", params.data.jobId);
  if (req.tenantId) jobQ = jobQ.eq("tenant_id", req.tenantId);
  const { data, error } = await jobQ.maybeSingle();
  if (error) { res.status(500).json({ error: error.message }); return; }
  if (!data) { res.status(404).json({ error: "No burner setup record for this job" }); return; }
