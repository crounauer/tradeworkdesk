...
import { CreateFireValveTestRecordBody, GetFireValveTestRecordParams, GetFireValveTestRecordResponse, UpdateFireValveTestRecordParams, UpdateFireValveTestRecordBody, UpdateFireValveTestRecordResponse, GetFireValveTestRecordByJobParams, GetFireValveTestRecordByJobResponse, } from "@workspace/api-zod";
const router = Router();
...
router.post("/fire-valve-test-records", requireAuth, async (req, res) => {
    ...
    const { data, error } = await supabaseAdmin.from("fire_valve_test_records").insert(parsed.data).select().single();
    if (error) {
...
router.get("/fire-valve-test-records/:id", requireAuth, async (req, res) => {
    ...
    if (error || !data) {
        res.status(404).json({ error: "Fire valve test record not found" });
        return;
    }
...
router.patch("/fire-valve-test-records/:id", requireAuth, async (req, res) => {
...
router.get("/fire-valve-test-records/job/:jobId", requireAuth, async (req, res) => {
    ...
    if (!data) {
        res.status(404).json({ error: "No fire valve test record for this job" });
        return;
    }
