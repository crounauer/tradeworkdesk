...
import { ListJobsQueryParams, ListJobsResponse, CreateJobBody, GetJobParams, GetJobResponse, UpdateJobParams, UpdateJobBody, UpdateJobResponse, DeleteJobParams, } from "@workspace/api-zod";
const router = Router();
router.get("/jobs", requireAuth, async (req, res) => {
    ...
    if (query.success) {
        ...
        if (query.data.job_type)
            q = q.eq("job_type", query.data.job_type);
        if (query.data.technician_id)
            q = q.eq("assigned_technician_id", query.data.technician_id);
        if (query.data.customer_id)
            q = q.eq("customer_id", query.data.customer_id);
        if (query.data.property_id)
            q = q.eq("property_id", query.data.property_id);
        if (query.data.date_from)
            q = q.gte("scheduled_date", query.data.date_from);
        if (query.data.date_to)
            q = q.lte("scheduled_date", query.data.date_to);
    }
...
router.post("/jobs", requireAuth, requireRole("admin", "office_staff"), async (req, res) => {
...
router.get("/jobs/:id", requireAuth, async (req, res) => {
    ...
    const filesWithUrls = await Promise.all((filesRes.data || []).map(async (f) => {
    ...
    const sigsWithUrls = await Promise.all((sigsRes.data || []).map(async (s) => {
...
router.patch("/jobs/:id", requireAuth, requireRole("admin", "office_staff"), async (req, res) => {
