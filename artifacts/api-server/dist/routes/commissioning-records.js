...
import { CreateCommissioningRecordBody, GetCommissioningRecordParams, GetCommissioningRecordResponse, UpdateCommissioningRecordParams, UpdateCommissioningRecordBody, UpdateCommissioningRecordResponse, GetCommissioningRecordByJobParams, GetCommissioningRecordByJobResponse, } from "@workspace/api-zod";
const router = Router();
...
router.get("/commissioning-records", requireAuth, async (req, res) => {
...
router.post("/commissioning-records", requireAuth, async (req, res) => {
...
router.get("/commissioning-records/:id", requireAuth, async (req, res) => {
    ...
    if (error || !data) {
        res.status(404).json({ error: "Commissioning record not found" });
        return;
    }
...
router.put("/commissioning-records/:id", requireAuth, async (req, res) => {
...
router.get("/jobs/:jobId/commissioning-record", requireAuth, async (req, res) => {
    ...
    if (!data) {
        res.status(404).json({ error: "No commissioning record for this job" });
        return;
    }
