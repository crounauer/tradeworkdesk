...
import { CreateOilLineVacuumTestBody, GetOilLineVacuumTestParams, GetOilLineVacuumTestResponse, UpdateOilLineVacuumTestParams, UpdateOilLineVacuumTestBody, UpdateOilLineVacuumTestResponse, GetOilLineVacuumTestByJobParams, GetOilLineVacuumTestByJobResponse, } from "@workspace/api-zod";
const router = Router();
...
router.post("/oil-line-vacuum-tests", requireAuth, async (req, res) => {
...
router.get("/oil-line-vacuum-tests/:id", requireAuth, async (req, res) => {
    ...
    if (error || !data) {
        res.status(404).json({ error: "Oil line vacuum test not found" });
        return;
    }
...
router.patch("/oil-line-vacuum-tests/:id", requireAuth, async (req, res) => {
...
router.get("/oil-line-vacuum-tests/job/:jobId", requireAuth, async (req, res) => {
    ...
    if (!data) {
        res.status(404).json({ error: "No oil line vacuum test for this job" });
        return;
    }
