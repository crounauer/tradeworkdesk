...
import { CreateBreakdownReportBody, GetBreakdownReportParams, GetBreakdownReportResponse, UpdateBreakdownReportParams, UpdateBreakdownReportBody, UpdateBreakdownReportResponse, GetBreakdownReportByJobParams, GetBreakdownReportByJobResponse, } from "@workspace/api-zod";
const router = Router();
...
router.post("/breakdown-reports", requireAuth, async (req, res) => {
...
router.get("/breakdown-reports/:id", requireAuth, async (req, res) => {
...
router.patch("/breakdown-reports/:id", requireAuth, async (req, res) => {
...
router.get("/breakdown-reports/job/:jobId", requireAuth, async (req, res) => {
    ...
    if (!data) {
        res.status(404).json({ error: "No breakdown report for this job" });
        return;
    }
