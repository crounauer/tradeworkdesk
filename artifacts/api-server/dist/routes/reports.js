...
import { GetUpcomingServicesResponse, GetOverdueServicesResponse, GetCompletedByTechnicianQueryParams, GetCompletedByTechnicianResponse, } from "@workspace/api-zod";
const router = Router();
router.get("/reports/upcoming-services", requireAuth, requireRole("admin", "office_staff"), async (_req, res) => {
...
router.get("/reports/overdue-services", requireAuth, requireRole("admin", "office_staff"), async (_req, res) => {
    const today = new Date().toISOString().split("T")[0];
    const { data, error } = await supabaseAdmin
        .from("appliances")
        .select("id, manufacturer, model, serial_number, next_service_due, properties(id, address_line1, customer_id, customers(id, first_name, last_name))")
...
router.get("/reports/completed-by-technician", requireAuth, requireRole("admin", "office_staff"), async (req, res) => {
