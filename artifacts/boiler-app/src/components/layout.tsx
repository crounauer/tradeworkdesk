        ...
        <div className="px-4 py-6 flex-1 overflow-y-auto space-y-1">
          ...
          {isAdmin && (
            <div className="pt-4 mt-3 border-t border-border/50">
              <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider px-3 mb-2">Admin</p>
          ...
          <div className="p-4 space-y-2">
            ...
            {isAdmin && (
              <div className="pt-3 mt-2 border-t border-border/50">
                <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider px-4 mb-2">Admin</p>
                {adminNavItems.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className={cn(
                    "flex items-center gap-3 px-4 py-3 rounded-xl text-base font-medium",
                    location.startsWith(item.href) ? "bg-primary/10 text-primary" : "text-foreground"
                  )}>
                    <item.icon className="w-5 h-5" />
                    {item.label}
                  </Link>
