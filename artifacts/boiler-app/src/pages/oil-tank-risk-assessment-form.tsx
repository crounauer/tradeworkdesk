...
export default function OilTankRiskAssessmentForm() {
  ...
  const handleExportPdf = async () => {
    try {
      const res = await fetch(`${import.meta.env.BASE_URL}api/jobs/${jobId}/forms/oil_tank_risk_assessment/${existingRecord!.id}/pdf`);
      if (!res.ok) throw new Error("Failed to generate PDF");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `oil-tank-risk-assessment-${jobId?.slice(0, 8)}.pdf`;
      a.click();
