              ...
              {showDeleteConfirm && (
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="destructive" type="button" size="sm" disabled={isDeleting} onClick={async () => {
                    setIsDeleting(true);
                    try {
                      await customFetch(`${import.meta.env.BASE_URL}api/combustion-analysis-records/${existingRecord!.id}`, { method: "DELETE" });
                      toast({ title: "Deleted", description: "Combustion analysis record deleted" });
                      setLocation(`/jobs/${jobId}`);
                    } catch (e: unknown) {
