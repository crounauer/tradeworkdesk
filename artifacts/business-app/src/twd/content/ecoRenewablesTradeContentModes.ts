...
import { ecoRenewablesTradePages } from "../templates/ecoRenewablesTrade.pages";
...
function getPageForMode(pageSlug: string, mode: ContentMode): TemplatePage {
  const source = ecoRenewablesTradePages[pageSlug as keyof typeof ecoRenewablesTradePages];

  if (!source) {
    throw new Error(`Missing eco-renewables trade page for slug "${pageSlug}"`);
  }
...
function buildSeedFile(mode: ContentMode): ContentSeedFile {
  ...
  for (const pageSlug of Object.keys(ecoRenewablesTradePages)) {
  ...
  return {
    template: "eco-renewables-trade",
    mode,
    pages,
  };
}

export const ecoRenewablesTradeContentModeDescriptors: ContentModeDescriptor[] = [
  {
    mode: "demo",
    label: "Demo content",
...
export const ecoRenewablesTradeContentModesManifest: ContentModesManifest = {
  template: "eco-renewables-trade",
  defaultMode: "demo",
  modes: ecoRenewablesTradeContentModeDescriptors.map((mode) => ({
    ...mode,
    file: `${mode.mode}.json`,
  })),
};

export const ecoRenewablesTradeContentSeeds: Record<ContentMode, ContentSeedFile> = {
  demo: buildSeedFile("demo"),
  empty: buildSeedFile("empty"),
  ai: buildSeedFile("ai"),
};

export function getEcoRenewablesTradePageByMode(pageSlug: string, mode: ContentMode): TemplatePage {
  return getPageForMode(pageSlug, mode);
}

export function getAvailableEcoRenewablesTradeContentModes(): ContentMode[] {
