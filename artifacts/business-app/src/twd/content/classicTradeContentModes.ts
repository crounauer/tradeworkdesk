...
import { classicTradePages } from "../templates/classicTrade.pages";
...
function getPageForMode(pageSlug: string, mode: ContentMode): TemplatePage {
  const source = classicTradePages[pageSlug as keyof typeof classicTradePages];

  if (!source) {
    throw new Error(`Missing classic trade page for slug "${pageSlug}"`);
  }
...
function buildSeedFile(mode: ContentMode): ContentSeedFile {
  ...
  for (const pageSlug of Object.keys(classicTradePages)) {
  ...
  return {
    template: "classic-trade",
    mode,
    pages,
  };
}

export const classicTradeContentModeDescriptors: ContentModeDescriptor[] = [
  {
    mode: "demo",
    label: "Demo content",
...
export const classicTradeContentModesManifest: ContentModesManifest = {
  template: "classic-trade",
  defaultMode: "demo",
  modes: classicTradeContentModeDescriptors.map((mode) => ({
    ...mode,
    file: `${mode.mode}.json`,
  })),
};

export const classicTradeContentSeeds: Record<ContentMode, ContentSeedFile> = {
  demo: buildSeedFile("demo"),
  empty: buildSeedFile("empty"),
  ai: buildSeedFile("ai"),
};

export function getClassicTradePageByMode(pageSlug: string, mode: ContentMode): TemplatePage {
  return getPageForMode(pageSlug, mode);
}

export function getAvailableClassicTradeContentModes(): ContentMode[] {
