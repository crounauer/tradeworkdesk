...
import { modernTradePages } from "../templates/modernTrade.pages";
...
function getPageForMode(pageSlug: string, mode: ContentMode): TemplatePage {
  const source = modernTradePages[pageSlug as keyof typeof modernTradePages];

  if (!source) {
    throw new Error(`Missing modern trade page for slug "${pageSlug}"`);
  }
...
function buildSeedFile(mode: ContentMode): ContentSeedFile {
  ...
  for (const pageSlug of Object.keys(modernTradePages)) {
  ...
  return {
    template: "modern-trade",
    mode,
    pages,
  };
}

export const modernTradeContentModeDescriptors: ContentModeDescriptor[] = [
  {
    mode: "demo",
    label: "Demo content",
...
export const modernTradeContentModesManifest: ContentModesManifest = {
  template: "modern-trade",
  defaultMode: "demo",
  modes: modernTradeContentModeDescriptors.map((mode) => ({
    ...mode,
    file: `${mode.mode}.json`,
  })),
};

export const modernTradeContentSeeds: Record<ContentMode, ContentSeedFile> = {
  demo: buildSeedFile("demo"),
  empty: buildSeedFile("empty"),
  ai: buildSeedFile("ai"),
};

export function getModernTradePageByMode(pageSlug: string, mode: ContentMode): TemplatePage {
  return getPageForMode(pageSlug, mode);
}

export function getAvailableContentModes(): ContentMode[] {
