...
import { boldIndustrialTradePages } from "../templates/boldIndustrialTrade.pages";
...
function getPageForMode(pageSlug: string, mode: ContentMode): TemplatePage {
  const source = boldIndustrialTradePages[pageSlug as keyof typeof boldIndustrialTradePages];

  if (!source) {
    throw new Error(`Missing bold industrial trade page for slug "${pageSlug}"`);
  }
...
function buildSeedFile(mode: ContentMode): ContentSeedFile {
  ...
  for (const pageSlug of Object.keys(boldIndustrialTradePages)) {
  ...
  return {
    template: "bold-industrial-trade",
    mode,
    pages,
  };
}

export const boldIndustrialTradeContentModeDescriptors: ContentModeDescriptor[] = [
  {
    mode: "demo",
    label: "Demo content",
...
export const boldIndustrialTradeContentModesManifest: ContentModesManifest = {
  template: "bold-industrial-trade",
  defaultMode: "demo",
  modes: boldIndustrialTradeContentModeDescriptors.map((mode) => ({
    ...mode,
    file: `${mode.mode}.json`,
  })),
};

export const boldIndustrialTradeContentSeeds: Record<ContentMode, ContentSeedFile> = {
  demo: buildSeedFile("demo"),
  empty: buildSeedFile("empty"),
  ai: buildSeedFile("ai"),
};

export function getBoldIndustrialTradePageByMode(pageSlug: string, mode: ContentMode): TemplatePage {
  return getPageForMode(pageSlug, mode);
}

export function getAvailableBoldIndustrialTradeContentModes(): ContentMode[] {
