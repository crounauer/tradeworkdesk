...
import { cleanMinimalTradePages } from "../templates/cleanMinimalTrade.pages";
...
function getPageForMode(pageSlug: string, mode: ContentMode): TemplatePage {
  const source = cleanMinimalTradePages[pageSlug as keyof typeof cleanMinimalTradePages];

  if (!source) {
    throw new Error(`Missing clean-minimal trade page for slug "${pageSlug}"`);
  }
...
function buildSeedFile(mode: ContentMode): ContentSeedFile {
  ...
  for (const pageSlug of Object.keys(cleanMinimalTradePages)) {
  ...
  return {
    template: "clean-minimal-trade",
    mode,
    pages,
  };
}

export const cleanMinimalTradeContentModeDescriptors: ContentModeDescriptor[] = [
  {
    mode: "demo",
    label: "Demo content",
...
export const cleanMinimalTradeContentModesManifest: ContentModesManifest = {
  template: "clean-minimal-trade",
  defaultMode: "demo",
  modes: cleanMinimalTradeContentModeDescriptors.map((mode) => ({
    ...mode,
    file: `${mode.mode}.json`,
  })),
};

export const cleanMinimalTradeContentSeeds: Record<ContentMode, ContentSeedFile> = {
  demo: buildSeedFile("demo"),
  empty: buildSeedFile("empty"),
  ai: buildSeedFile("ai"),
};

export function getCleanMinimalTradePageByMode(pageSlug: string, mode: ContentMode): TemplatePage {
  return getPageForMode(pageSlug, mode);
}

export function getAvailableCleanMinimalTradeContentModes(): ContentMode[] {
