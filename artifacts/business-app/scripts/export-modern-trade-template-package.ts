...
const templateSlug = 'modern-trade';
const templateName = 'Modern Trade';
const packageName = 'modern-trade-template-package';
...
const pagePaths: Record<string, string> = {
  ...
  about: '/about',
  services: '/services',
  'service-detail': '/services/oil-boiler-servicing',
  ...
  contact: '/contact',
  'blog-index': '/blog',
  'blog-post': '/blog/oil-boiler-service',
...
const seoDescriptions: Record<string, string> = {
  home: 'Reliable plumbing and heating services from a trusted local trade business.',
  about: 'Learn more about this local plumbing and heating business.',
  services: 'Browse plumbing and heating services including servicing, repairs and installations.',
  'service-detail': 'Oil boiler servicing information, benefits and booking details.',
  ...
  gallery: 'View recent plumbing and heating work examples.',
  faq: 'Answers to common plumbing and heating service questions.',
  contact: 'Contact this local trade business to request a quote or ask a question.',
  'blog-index': 'Helpful plumbing and heating advice for homeowners.',
  'blog-post': 'Advice article about oil boiler servicing.',
...
function exportPages() {
  const pagesManifest = pageOrder.map((slug) => {
    const page = modernTradePages[slug];
...
function exportTemplateJson() {
  writeJson(path.join(packageDir, 'templates', templateSlug, 'template.json'), {
    ...
    style: 'modern',
    description:
      'A modern block-based template for UK plumbing, heating and local trade businesses.',
    industries: [
      'Plumbing',
      'Heating',
    ...
    defaultPage: 'home',
    pages: pageOrder,
    requiredBlocks: [...new Set(pageOrder.flatMap((slug) => modernTradePages[slug].blocks.map((block) => block.type)))],
    source: {
      type: 'storybook-recipes',
      recipeFile: 'src/twd/templates/modernTrade.pages.ts',
...
-- Example seed for the Modern Trade template.
...
- \`src/twd/templates/modernTrade.pages.ts\`
...
function main() {
  ...
  console.log('');
  console.log('Modern Trade template package exported.');
  console.log('');
