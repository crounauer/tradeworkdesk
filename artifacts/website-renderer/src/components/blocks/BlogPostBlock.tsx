...
export default function BlogPostBlock({ content, site, page }: Props) {
  ...
  return (
    <article style={{ maxWidth: 800, margin: "0 auto", padding: "64px 24px" }}>
      ...
      {safeBody && <div style={{ color: "#374151", lineHeight: 1.85 }} dangerouslySetInnerHTML={{ __html: safeBody }} />}
      {!safeBody && post?.excerpt && <p style={{ color: "#374151", lineHeight: 1.85 }}>{post.excerpt}</p>}
