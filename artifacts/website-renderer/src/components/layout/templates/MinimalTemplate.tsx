/**
 * Template: minimal
 * Style: Ultra-clean, lots of whitespace, sans-serif heavy.
...
export default function MinimalTemplate({ site, children, basePath, previewToken }: TemplateLayoutProps) {
  const { website, company } = site;
  const navPages = site.pages.filter((p) => p.show_in_nav);

  const theme = {
    ...(website.theme as Record<string, string>),
    nav_background: "#f9fafb",
    nav_text: "#374151",
