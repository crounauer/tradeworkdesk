...
- **Plan Feature Gating:** Base plan + individually selectable add-ons model. Features derived from union of base plan `features` JSONB + active add-on `feature_keys`. DB tables: `addons` (catalog with name, feature_keys, pricing, Stripe price IDs) and `tenant_addons` (per-tenant activation). Super admin manages add-on catalog via `/platform/addons`. Tenants select add-ons during registration and manage them from billing page. Stripe webhook `customer.subscription.updated` syncs add-on state from subscription items. Legacy tiered plans continue working during transition (features union with add-ons).
...
- **Customer Login Portal:** A separate `/portal/*` frontend with its own auth flow (`PortalAuthProvider`, `usePortalAuth`) lets homeowners/landlords register via invite link, log in, and view read-only data scoped to their customer record: dashboard, properties, appliances, service history, job detail with service records/completion reports, and PDF certificate downloads. Admin controls on the customer detail page allow sending portal invitations and toggling access. Backend: `customer_portal_users` table, `requireCustomerAuth` middleware in `portal.ts`, admin endpoints in `customers.ts` (portal-invite, portal-status, portal-toggle). Migration: `supabase/patches/patch-024-customer-portal-users.sql`.
