...
- **Registration:** Supports multi-step company signup and invite-code registration. Includes a "Sole Trader" / "Company" toggle — sole traders get a simplified form (trading name optional, defaults to full name) and their profile functions as both admin and sole technician.
- **Sole Trader Mode:** Tenants with `company_type = 'sole_trader'` get a streamlined experience: Team/Invite Codes nav items are hidden, job creation auto-assigns the sole trader as technician (hiding the dropdown), and a "Switch to Company Mode" option is available in Company Settings. The `useIsSoleTrader()` hook provides consistent checks across components. Migration: `supabase/patch-006-sole-trader.sql`.
